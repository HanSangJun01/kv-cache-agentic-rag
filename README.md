...
- **확증 편향 방지 전략** (코드 `prompts/criteria.py: BIAS_STRATEGIES` → 보고서 작성 프롬프트에 입력되어 보고서 6장에 7개 항목으로 반영)
  1. 질의 쌍 구성: 웹 검색을 하는 기준(M1~M3·D4)은 '채택·성과' 질의와 '한계·제약(사실)' 질의 쌍으로 수행 (D1~D3 는 논문 근거가 없을 때만 fallback 질의), 반응·의견 질의(criticism/review/opinion) 미사용
  2. 반대 근거 강제: 모든 기준 평가에 `counter_evidence` 필수 필드 (반대 근거가 없으면 '확인된 반대 근거 없음'을 채우도록 프롬프트로 지시)
...
| Retrieval | FAISS + BM25 Hybrid (RRF, `tech` 필터) — Hit Rate@5 **0.567**, MRR@10 **0.507** (60문항, 아래 검색 품질) |
| Embedding | **BAAI/bge-m3** (오픈소스, 로컬 추론) |
| Web Search | Tavily |
| PDF | PyMuPDF(로딩), Markdown + WeasyPrint(보고서 렌더링) |

### Embedding 모델 선정 — `BAAI/bge-m3` 고정
영어 기술 논문을 원문으로, 한국어 평가 기준·보고서를 산출물로 다루는 과제 상황에 맞춰 오픈소스 후보 3종의 특징을 비교해 선정했다 (리더보드 순위는 기준에서 제외, `rag/pipeline.py: EMBED_MODEL`).

| 항목 | **bge-m3** ✅ | Qwen3-Embedding-0.6B | bge-small-en-v1.5 |
|---|---|---|---|
| 언어 | 다국어 (한국어 포함) | 다국어 (한국어 포함) | 영어 전용 |
| 한국어 "도입 용이성과 품질 유지 관점의 한계" | 9토큰, 단어 단위 | 17토큰, 바이트 단위 분해 | 34토큰, 자모 분해 + `[UNK]` |
| 규모 · 다운로드 | 약 5.7억 · 2.27GB | 약 6억 · 1.19GB | 약 0.3억 · 0.13GB |
| 질의 지시문 · 로컬 인덱싱(240청크)* | 불필요 · 46초 | 필요 · 104초 | 선택 · 29초 |
| 라이선스 | MIT | Apache-2.0 | MIT |

\* 토큰 수·인덱싱 시간은 선정 당시 로컬 측정값 (인덱싱 시간은 모델 로딩 포함). 모델을 고정한 뒤 `rag/evaluate.py` 는 선정 모델의 검색 품질만 측정한다.

- 한국어·영어를 모두 단어 단위로 다뤄, 한국어 기준 용어와 영어 원문을 함께 다루는 이 과제에 맞음
- bge-small 은 한국어를 자모로 분해하고 `[UNK]` 가 생겨 제외, Qwen3 는 질의마다 지시문이 필요하고 인덱싱이 2.3배 느림
- MIT 라이선스 · 로컬 무료 실행으로 경제성 충족 (감수: 다운로드 2.27GB, 최초 1회)

**검색 품질** (`python -m rag.evaluate`, 60문항): Hybrid Hit@5 **0.567** · MRR@10 **0.507** (Dense 0.517 · 0.403, BM25 0.617 · 0.532) — 질문이 원문 문장에서 생성돼 BM25 에 유리한 편향이 있어, 추상적인 에이전트 질의에 대응하도록 Hybrid 유지
...
| 📊 시장성 평가 `market_eval` | `technologies`, `tech_profiles`, `domain` | `market_eval`(M1~M3 × 2기술), `sources`(WM*), `trace` | Agentic RAG + **웹 검색** + Reflection | 시장 규모·채택·생태계 등급. 웹이 주근거, 논문은 구현·통합 수준 확인 |
...
| ⚖️ 평가 종합 `synthesis` | `tech_profiles`, `market_eval`, `domain_eval`, `domain` | `synthesis` | 없음 | 일치 / 상충 / 기술 간 인식 차이 / 보완 가능성 / 유의점 (새 사실 생성 금지) |
| 📝 보고서 생성 `report_writer` | 위 전부 + `sources`, 재작성 시 `review`·`report_md`·`revision_count` | `report_md`, `revision_count` | 없음 | 목차 구성·서술, 태그 → 번호·REFERENCE 자동 변환 |
| ✅ 보고서 검토 `report_reviewer` | `report_md`, `sources`, `technologies`(수치 대조용 논문 원문), `market_eval`·`domain_eval`·`synthesis`(Judge 대조용) | `review` | 규칙 검사기 + Judge LLM | critical 있으면 revise, 없으면 approve, 한도 도달 시 unverified |
| 💾 저장 `save_outputs` (에이전트 아님) | `report_md`, `review`, `revision_count`, `sources`, `trace` | `outputs` | MD/PDF export | 단순 I/O |

- 기술 조사·시장성·도메인 에이전트는 기술 2건(SW ∥ HW)을 **노드 내부에서 병렬** 처리하고 `{"sw": ..., "hw": ...}` 로 한 키에 담음 (노드 수를 늘리지 않음). 종합·보고서 생성은 두 기술을 한 번에 다룸
...
| `sources` | list[dict] | `operator.add` | 조사·시장·도메인 | 출처 레지스트리 `{id, kind, ref_type, tech, title, url, site, date, citation}` (`ref_type` = 논문/특허/기타, REFERENCE 소제목 기준) — 웹 ID 는 노드별 prefix `WM*`/`WD*` |
...
├── tests/test_offline.py   # LLM 없이 도는 결정론 검사 (인용 변환·수치 대조·검토 규칙·분기·출처 ID·웹 질의·REFERENCE 형식)
